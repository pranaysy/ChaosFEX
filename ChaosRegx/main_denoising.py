# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 00:13:33 2020
...
from load_data import get_data_signal_denoise
...
DATA_NAME = 'signal_denoising'
X_TRAIN, Y_TRAIN, X_TEST, Y_TEST = get_data_signal_denoise(DATA_NAME)
...
for K in K1:
    ...
    if np.max([X_TRAIN.shape[1], Y_TRAIN.shape[1]]) == 1:
        ...
        plt.plot(X_TEST,Y_TRAIN,'-k', markersize = 4, label = 'actual value')
        plt.plot(X_TEST,Y_PRED,'*-r', markersize = 5, label = 'predicted value')
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.grid(True)
        plt.xlabel('X', fontsize=20)
        plt.ylabel('Prediction', fontsize=20)
        plt.legend(bbox_to_anchor=(0.69,0.88),fontsize = 17)
